...
router.get('/:id', async (req, res) => {
  ...
  if (!vehicle) {
    res
      ...
      .json({
        success: false,
        message: 'The Vehicle with the given ID not found!',
      });
  }
...
router.get('/get/bycm/:capacity', async (req, res) => {
  const vehicle = await VehiclesModel.find({
    capacityInCm: { $gte: parseInt(req.params.capacity) },
  });
  if (!vehicle) {
    res
      .status(500)
...
router.get('/get/byton/:capacity', async (req, res) => {
  const vehicle = await VehiclesModel.find({
    capacityInTon: { $gte: parseInt(req.params.capacity) },
  });
  if (!vehicle) {
    res
      .status(500)
...
router.delete('/:id', (req, res) => {
  VehiclesModel.findByIdAndRemove(req.params.id)
    .then((vehicle) => {
      if (vehicle) {
        return res
          .status(200)
          .json({ success: true, message: 'the Vehicle is deleted' });
      } else {
        return res
          .status(404)
          .json({ success: false, message: 'the Vehicle not found!' });
      }
    })
    .catch((err) => {
